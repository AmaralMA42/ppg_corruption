...
def calcula_payoff(sitio, estrategia_, viz_, params_):
    ...
    payoff_sitio += public_good_benefit(Nc,Nd,Np, params_) # matriz_pay_[int(Nc),int(Nd)] # r * (Nc + Np - sigma * Np) / G
# jogo centrado em cada um dos 4 vizinhos
    ...
    if estrategia_[sitio] == 2:
        payoff_sitio = payoff_sitio - 5*c + 5*sigma
    return payoff_sitio
