...
def calcula_payoff(sitio, estrategia, viz, params):
    ...
    alpha = params[5]
    payoff_sitio = 0
    # jogo centrado no sítio
    Nc, Nd, Np = conta_estrat(sitio, viz, estrategia)
    estrat_atual = estrategia[sitio]
    payoff_sitio += public_good_benefit(Nc, Nd, Np, params)
    # matriz_pay_[int(Nc),int(Nd)] # r * (Nc + Np - sigma * Np) / G ?????
    if estrat_atual == 0:
        #        payoff_sitio = payoff_sitio - 5 * c
        if Nc > 0:
            payoff_sitio = payoff_sitio - c + alpha * sigma * c * (Np / Nc)
        else:
            payoff_sitio = payoff_sitio - c
    if estrat_atual == 2:
        #        payoff_sitio = payoff_sitio - 5 * c + 5 * sigma * c
        if Nc > 0:
            payoff_sitio = payoff_sitio - c + (1 - alpha) * sigma * c
        else:
            payoff_sitio = payoff_sitio - c + sigma * c
