...
def inicia_estrategias(estrategia, total_jog, L, cond_ini):
    ...
    if cond_ini == 0:   #total random
        for jogador_atual in range(total_jog):  # Lembrete: for in range vai até total_jog-1!!
            estrategia[jogador_atual] = np.random.randint(3)  # Estado inicial do jogador como aleatório
    elif cond_ini == 1:  # linhas de estyratégias
        for i in range(0, L):
            for j in range(0, L):
                if i <= L//3:
                    estrategia[i * L + j] = 0
        ...
        for i in range(0, L):
            for j in range(0, L):
                if i < tgangle*j  and i < L//2:
                    estrategia[j * L + i] = 2
    elif cond_ini == 4:     # classico C VS D
        for jogador_atual in range(total_jog):  # Lembrete, for in range vai até total_jog-1!!
